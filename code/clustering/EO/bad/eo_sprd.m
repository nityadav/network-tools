function [best_labels,maxQ] = eo_clustering(adj,trials)
...
function lbl = recursive_eo(adj, alpha, prev_maxQ, true_index)
	...
	maxQ = 0;
	best_labels = lbl;
	% preprocessing step to speed up fitness function
	%sim_mat = get_nbh_matrix(adj);
	%sim_mat = get_ppr_matrix(adj);
	...
	for step = 1:n*alpha
		...
		fitness = norm_mod_fitness(adj,labels);
		%fitness = mod_fitness(adj,labels);
		%fitness = nbh_fitness(sim_mat,labels);
		%fitness = perm_fitness(adj,labels);
		%fitness = from_ppr_fitness(sim_mat, labels);
		%fitness = to_ppr_fitness(sim_mat, labels);
