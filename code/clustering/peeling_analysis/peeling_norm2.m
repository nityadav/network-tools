function [Jtable, progress] = peeling_norm2(adj,truth,trials)
	n = size(adj,1);
...
function clustering = recursive_nbr(adj, m, identity, sim_mat, limit)
	...
	if found
		peeled_identity = identity(peeled);
		unpeeled_identity = identity(unpeeled);
		% make the recursive call
		peeled_clustering = recursive_nbr(adj(peeled,peeled), m, peeled_identity, sim_mat(peeled,peeled), limit);
		unpeeled_clustering = recursive_nbr(adj(unpeeled,unpeeled), m, unpeeled_identity, sim_mat(unpeeled,unpeeled), limit);
		clustering(peeled_identity) = peeled_clustering(peeled_identity);
		clustering(unpeeled_identity) = unpeeled_clustering(unpeeled_identity);
	else
