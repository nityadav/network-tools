function [best_clustering,time] = unbr_prb(adj)
	tic
	...
	for w = 1:length(weights)
		...
		for l = 1:length(limits)
			...
			% compare with best obtained so far
			Q = get_stability(adj,clustering,k);
			if Q > best_Q
				best_Q = Q;
...
function stability = get_stability(adj, labels, k)
	sim_mat = get_nbr_sim(adj,4);
	stability = 0;
	for i = 1:k
		mem = find(labels==i);
