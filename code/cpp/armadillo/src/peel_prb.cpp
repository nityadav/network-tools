...
int main(int argc, char** argv) {
	...
	/* for quality testing using nbr affinity stability
	mat qual_mat = adj + (adj2/deg_mat);
	double stability;
	double max_stability = 0;
	uvec best_clustering; */

	// for quality testing using modularity
	double twiceM = sum(deg);
	mat modularity_mat = adj - (deg_rows % trans(deg_rows))/twiceM;
	double max_modularity = -0.5;
	uvec best_clustering;

	for (uword w = 0; w < weights.n_elem; w++) {
		mat nbr_mat = weights(w)*adj + (adj2/deg_mat);

		for (uword l = 0; l < limits.n_elem; l++) {
			
			// clustering process begins
			uvec clustering(num_nodes,fill::zeros);
			mat comm_mat(num_nodes,num_nodes,fill::zeros);
			int k = 1;
			...
			while(!residual.is_empty()) {
				...
				vec sort_norm_deg = sort(res_deg)/sum(res_deg);
				uvec sort_order = sort_index(res_deg);
				...
				for (uword v=0; v < num_left; v++) {
					...
					mat mem_mat = nbr_seed_selection(&subadj,seed,num_left,limits(l));
				...
				if (!found) {
					...
					comm_mat(residual,ones<uvec>(1)*k - 1).fill(1);
					break;
				}
				else {
					if (comm1.n_elem < comm2.n_elem) {
						...
						comm_mat(comm1,ones<uvec>(1)*k - 1).fill(1);
					}
					else {
						residual = comm1;
						clustering(comm2).fill(k);
						comm_mat(comm2,ones<uvec>(1)*k - 1).fill(1);
					}
				}	
				k++;
			}
			// check if this is the best clustering
			mat sliced_comm_mat = comm_mat.head_cols(k);
			double modularity = trace(trans(sliced_comm_mat)*modularity_mat*sliced_comm_mat)/twiceM;
